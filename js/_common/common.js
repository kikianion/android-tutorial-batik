...
var lastStockRenderer1 = function(instance, td, row, col, prop, value, cellProperties) {
    td.innerHTML = value;
    ...
    if (row < 0) {
        ...
        // debugger;
        val1 = instance.getDataAtCell(row - 1, col + 1);
        val2 = instance.getDataAtCell(row, col - 1);
        valFinal = parseInt(val1) - parseInt(val2);
    }

    var val4 = instance.getDataAtCell(row, col + 1);
    var color = "";
    if (valFinal != val4 && !isNaN(valFinal)) {
        color = "red";
    }

    td.innerHTML = "<div style='color:" + color + " ;white-space:nowrap'>" + valFinal + "</div>";
    // instance.setDataAtCell(row, col, valFinal, "loadData");
    // value = valFinal;
    if (!value && isNaN(valFinal)) td.innerHTML = '';

};

var diffStockRenderer1 = function(instance, td, row, col, prop, value, cellProperties) {
    td.innerHTML = value;
    ...
    var color = "";
    if (row > 0) {
        var prevRealStock = instance.getDataAtCell(row - 1, col - 1);
        var realStock = instance.getDataAtCell(row, col - 1);
        var buy = instance.getDataAtCell(row, col - 3);
        var countedStock = parseInt(prevRealStock) - parseInt(buy);

        // debugger;
        valFinal = realStock - countedStock;
...
function eb_download(data, fileName) {
    ...
    if (typeof a.download != "undefined" && DISABLE_LOCAL_DOWNLOAD == 0) {
        ...
        try {
            ...
            var blob = new Blob(
                [arraybuffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' }
            );
        } catch (e) {
